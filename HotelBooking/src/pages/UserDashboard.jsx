...
export default function UserDashboard() {
  ...
  const removeFromWishlist = async (roomTypeId) => {
    ...
    setWishlist((prev) =>
      prev.filter((rt) => (rt.uuid || rt.id) !== roomTypeId),
    );
  };
    ...
    <div className="py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-card border border-border rounded-3xl p-6 shadow-warm mb-8 flex items-center gap-5">
          <div className="w-16 h-16 rounded-2xl bg-primary/10 text-green-800 flex items-center justify-center font-display font-bold text-2xl shrink-0">
        ...
        <div className="flex gap-1 bg-muted rounded-xl p-1 mb-8 w-fit">
          {[
            ...
            >
              ...
              {count > 0 && (
                <span
                  className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${tab === id ? "bg-primary/10 text-green-800" : "bg-border text-muted-foreground"}`}
                >
                  {count}
                </span>
              )}
            ...
            {loadingBookings ? (
              <div className="space-y-4">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-28 bg-muted rounded-2xl animate-pulse border border-border"
                  />
                ))}
              </div>
            ) : bookings.length === 0 ? (
              <div className="text-center py-20 bg-card rounded-3xl border border-border shadow-warm-sm">
                ...
                <Link
                  to="/rooms"
                  className="inline-block mt-5 px-6 py-2.5 bg-primary text-white rounded-xl font-semibold text-sm hover:bg-primary-light transition-colors"
                >
                  Browse Rooms
                </Link>
              </div>
              ...
              <div className="space-y-4">
                {bookings.map((booking) => (
                  <div
                    key={booking.id}
                    className="bg-card rounded-2xl border border-border shadow-warm-sm p-5 flex flex-col sm:flex-row gap-5"
                  >
                    <div className="w-full sm:w-28 h-20 rounded-xl overflow-hidden bg-muted shrink-0">
                      <img
                        src={
                          booking.room?.room_type?.images?.[0] ||
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 flex-wrap mb-2">
                        ...
                        <span
                          className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold capitalize border ${STATUS_STYLES[booking.status] || STATUS_STYLES.pending}`}
                        >
            ...
            {loadingWishlist ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-52 bg-muted rounded-2xl animate-pulse border border-border"
                  />
                ))}
              </div>
            ) : wishlist.length === 0 ? (
              <div className="text-center py-20 bg-card rounded-3xl border border-border shadow-warm-sm">
                ...
                <p className="text-muted-foreground text-sm mt-1">
                  Save room types while browsing to find them here.
                </p>
                <Link
                  to="/rooms"
                  className="inline-block mt-5 px-6 py-2.5 bg-primary text-white rounded-xl font-semibold text-sm hover:bg-primary-light transition-colors"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
                {wishlist.map((roomType) => {
                  ...
                  return (
                    <div
                      key={rtId}
                      className="bg-card rounded-2xl border border-border shadow-warm-sm overflow-hidden group hover:-translate-y-0.5 hover:shadow-warm transition-all duration-300"
                    >
                      <Link to={`/rooms/${rtId}`}>
