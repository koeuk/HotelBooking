...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        api.get('/bookings', { params: { per_page: 5 } })
            .then(res => {
                const data = res.data.data || [];
                setRecentBookings(data);
                const all = res.data.data || [];
                const revenue = all.reduce((sum, b) => sum + (parseFloat(b.total_price) || 0), 0);
                setBookingStats({
    ...
    const quickActions = [
        { label: 'Manage Rooms', path: '/admin/rooms', icon: <BedDouble size={18} /> },
        { label: 'View Bookings', path: '/admin/bookings', icon: <CalendarCheck size={18} /> },
        { label: 'Hotel Settings', path: '/admin/hotel-settings', icon: <Settings size={18} /> },
    ];
    ...
    return (
        <div className="space-y-8">
            <header>
                ...
                <p className="text-slate-500 mt-1">Here's what's happening at your hotel today.</p>
            </header>
                            ...
                            <tbody>
                                {recentBookings.map(booking => (
                                    <tr key={booking.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="py-3 text-slate-500">{booking.check_in_date || booking.check_in || '—'}</td>
                                        <td className="py-3 text-slate-500">{booking.check_out_date || booking.check_out || '—'}</td>
                                        <td className="py-3">
                                            <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${STATUS_COLORS[booking.status] || STATUS_COLORS.pending}`}>
