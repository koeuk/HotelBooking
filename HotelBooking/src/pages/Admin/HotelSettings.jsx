    ...
    return (
        <div className="space-y-6 max-w-3xl">
            <header>
                ...
                <p className="text-slate-500 mt-1">View and update your hotel information.</p>
            </header>
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
                    ...
                    <div className="flex items-center gap-4 mb-8 pb-6 border-b border-slate-100">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-slate-900">{hotel?.name || 'Your Hotel'}</h2>
                            {hotel?.stars && (
                    ...
                    <form onSubmit={handleSave} className="space-y-5">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div>
                                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Hotel Name</label>
                                <input
                                    type="text"
                                    value={form.name || ''}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div>
                                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Check-in Time</label>
                                <input
                                    type="time"
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Check-out Time</label>
                                <input
                                    type="time"
                        ...
                        {hotel?.stars && (
                            <div>
                                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Star Rating (read-only)</label>
                                {renderStars(hotel.stars)}
                                <p className="text-xs text-slate-400 mt-1">Star rating is managed by administrators.</p>
                            </div>
                        )}
