...
- Redirect admin → /admin/dashboard, guest → /
...
### AuthController.php
- register() → create user (role=guest) → return token + UserResource
...
- RoomTypeResource: id, hotel (name), name, description, max_guests, price_per_night, images
...
- Email to admin: guest info, room, hotel, dates, price, link to admin panel
...
- Responsive HTML, hotel name header, guest details, booking details, CTA button
...
- Use <Badge> for role (admin=blue, guest=gray)
